...
```{r}
km_dp <- kmodes(random_df_country1, 3)
...
lca1_d <- poLCA(f, random_df1, nclass = 5)
mmc1_d <- mixmodCluster(random_df1, 5)
```

## ARI comparison 
```{r}
adjustedRandIndex(result1_dp_country$pred_class, km_dp$cluster)
adjustedRandIndex(result2_dp$pred_class, lca2_dp$cluster)
